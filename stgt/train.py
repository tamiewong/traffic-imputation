...
def train_model(config_path: str):
    ...
    print(f"[TRAIN][CHECK] Shape: E={E} edges, T={T} timesteps")
    window = int(cfg.get("window", 96))
    step   = int(cfg.get("step", 96))
    epochs = int(cfg.get("epochs", 20))
    print(f"[TRAIN][CFG] window={window}, step={step}, epochs={epochs}")

    if T == 0:
        print("[TRAIN][FATAL] T == 0 (no timestamps). Check your 'utd' date parsing / filtering.")
        return
    if window <= 0 or step <= 0:
        print("[TRAIN][FATAL] window/step must be positive.")
        return

    # 1) device
    device = torch.device("cuda" if torch.cuda.is_available() and str(cfg.get("use_cuda", False)).lower() in {"1","true","yes","on"} else "cpu")
    print(f"[TRAIN][CFG] device={device}", flush=True)
    cache_dir = paths.get("cache", os.path.join(paths["outputs"], "cache")); os.makedirs(cache_dir, exist_ok=True)
    out_dir = paths.get("outputs", "outputs"); os.makedirs(out_dir, exist_ok=True)
    formatted_datetime = datetime.now().strftime("%Y%m%d-%H%M%S")

    # 2a) build adjacency list
    adj_path = os.path.join(out_dir, f"adj_{formatted_datetime}.npy")
    ...
    print(f"[TRAIN][CHECK] neighbors dtype={neighbors.dtype}, shape={neighbors.shape}")
    mn = neighbors.min()
    mx = neighbors.max()
    neg = int((neighbors < 0).sum())
    oob = int((neighbors >= E).sum())
    print(f"[TRAIN][CHECK] min={mn}, max={mx}, E={E}, neg={neg}, oob={oob}", flush=True)

    # 3) LapPE (positional signal) smoothing
    if cfg["K_lappe"] > 0:
        ...
        U_lappe = np.zeros((E, 0), dtype=np.float32)
    print('[TRAIN] LapPE ok', flush=True)

    # 4) build row-normalized sparse adjacency
    ...
    print(f"[TRAIN][CHECK] sp_adj nnz={sp_adj._nnz()}", flush=True)

    print("[TRAIN][DEBUG] testing sparse mm...", flush=True)
    probe_mat = torch.randn(sp_adj.size(1), 4, device=device)  # (E,4)
    probe_out = torch.sparse.mm(sp_adj, probe_mat)
    print("[TRAIN][DEBUG] sparse mm ok ->", tuple(probe_out.shape), flush=True)
    ...
    print('[TRAIN] incidence csr ok', flush=True)

    # Model
    spatial_mode = str(cfg.get("spatial_mode", "sparsemm")).lower()
    model = STGT(
        ...
        sp_adj=sp_adj if spatial_mode=="sparsemm" else None
    ).to(device)
    print(f"[TRAIN][CFG] spatial_mode={spatial_mode}", flush=True)
    opt = torch.optim.AdamW(model.parameters(), lr=cfg.get("lr", 0.002), weight_decay=cfg.get("weight_decay", 0.0001))
    print('[TRAIN] model ok', flush=True)
    ...
    # Training loop
    print('[TRAIN] training loop...', flush=True)
    ...
    if probe is None:
        print("[TRAIN][FATAL] window_generator produced 0 windows. "
            "Reduce 'window', fix 'times', or check M_train/M_val building.")
        return
    else:
        _, Mb_tr_probe, _, _, _, s_probe, e_probe = probe
        print(f"[TRAIN][CHECK] window check ok -> First window indices: [{s_probe}:{e_probe}), "
            f"observed entries in train mask: {Mb_tr_probe.sum()}")

    last_avg_loss = float("nan")   # set BEFORE the try
    #ep = -1                        # epoch counter for safe saving
        ...
        for ep in range(epochs):
            ...
            for Xb, Mb_tr, Mb_val, hours, dows, s, e in window_generator(X_sparse, M_train, M_val, times, window, step):
                ...
                try:
                    ...
                    print("[TRAIN][ERROR] Exception during forward/backward:", e, flush=True)
                    traceback.print_exc()
                    raise

                print(f"[TRAIN] forward ok in {t1 - t0:.2f}s | loss {t2-t1:.2f}s | bwd {t3-t2:.2f}s | step {t4-t3:.2f}s", flush=True)
            ...
            print(f"[TRAIN][SAVE] checkpoint -> {os.path.abspath(ckpt_path)}"); sys.stdout.flush()

            # (Optional) quick val over one window:
            # You can add a validation loop similar to the train loop with Mb_val.

            if max_windows_debug and win_i >= max_windows_debug:
                print("[TRAIN][DEBUG] Stopping early after max_windows_debug windows.", flush=True)
                break

        if not any_batch:
            print("[TRAIN][WARN] No training windows generated. Check your time range, window, and step."); sys.stdout.flush()

    finally:
        # Save a final snapshot
        print('[TRAIN][SAVE] final snapshot...'); sys.stdout.flush()
        final_path = os.path.join(out_dir, f"stgt_{formatted_datetime}.pt")
        torch.save({"epoch": epochs, "model_state": model.state_dict(), "cfg": cfg}, final_path)
        summary_path = os.path.join(out_dir, f"train_summary_{formatted_datetime}.json")
        with open(summary_path, "w") as f:
            json.dump({"epochs": epochs, "train_loss": float(last_avg_loss)}, f, indent=2)
        print(f"[TRAIN][SAVE] final model -> {os.path.abspath(final_path)}")
        print(f"[TRAIN][SAVE] summary     -> {os.path.abspath(summary_path)}")
        sys.stdout.flush()
